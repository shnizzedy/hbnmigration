...
flowchart TD
    ...
    start@{ shape: circle, label: "ripple-to-redcap" } --> request_potential_participants@{ shape: procs, label: "(POST «Ripple»/export) × 2" } --> ripple_data@{ shape: diamond, label: "any participants marked \"Send to RedCap\"?" } -- no --> stop@{ shape: dbl-circ, label: "Stop" };
    ...
    update -- yes --> update_redcap@{ shape: rounded, label: "POST «REDCap»" } --> update_ripple@{ shape: procs, label: "(POST «Ripple»/import) × (1 or 2)" };
